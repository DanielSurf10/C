...
int insere_lista_inicio(Lista* li, struct aluno al) {
    ...
    no->dados = al;         // Coloca o dado no nó
...
int insere_lista_final(Lista* li, struct aluno al) {
    ...
    if ((*li) == NULL)                          // Se o a lista é vazia
        return insere_lista_inicio(li, al);         // Insere no começo e inerção feita
...
int insere_lista_indice(Lista* li, struct aluno al, int indice) {
    ...
    if (indice >= tamanho_Lista(li) - 1 || indice == -1)    // Se o indice for >= (tamanho da lista - 1) ou indice == -1
        return insere_lista_final(li, al);                      // Insere no final e inserção feita
    ...
    if (indice < 0) {                                       // Se o indice for menor que 0
        ...
        if (indice <= 0)                                        // Se o indice ainda for <= 0
            return insere_lista_inicio(li, al);                     // Insere no começo e inserção feita
    }
...
int consulta_lista_indice(Lista* li, struct aluno* al, int indice) {
    ...
    *al = aux->dados;                               // al recebe o valor do nó correspondente ao índice da lista
...
int consulta_lista_num(Lista* li, struct aluno* al, int num) {
    ...
    while (aux != NULL && aux->dados.matricula != num)      // Enquanto aux != NULL E matricula != num
        aux = aux->prox;

    if (aux == NULL)                                        // Se aux == NULL
        return 0;                                               // Valor não encontrado, retorna 0 (falhou)
    
    *al = aux->dados;                                       // al recebe o valor do nó correspondente ao num
...
void printaLista(Lista* li) {
    ...
    for (int i = 0; aux != NULL; i++) {
        printf("%2d | %10s | %d\n", i, aux->dados.nome, aux->dados.matricula);
        aux = aux->prox;
    }
}
